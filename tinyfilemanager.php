...
if ($use_auth && !isset($_SESSION[FM_SESSION_ID]['logged']) && isset($_COOKIE['fm_remember'])) {
    $cookie_data = base64_decode($_COOKIE['fm_remember']);
    if ($cookie_data && strpos($cookie_data, ':') !== false) {
        list($username, $hash) = explode(':', $cookie_data, 2);
        if (isset($auth_users[$username]) && $auth_users[$username] === $hash) {
            $_SESSION[FM_SESSION_ID]['logged'] = $username;
        } else {
            // Invalid cookie, delete it
            setcookie('fm_remember', '', time() - 3600, '/');
        }
    }
}
    ...
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        ...
        if (function_exists('password_verify')) {
            if (isset($auth_users[$_POST['fm_usr']]) && isset($_POST['fm_pwd']) && password_verify($_POST['fm_pwd'], $auth_users[$_POST['fm_usr']]) && verifyToken($_POST['token'])) {
                ...
                if (isset($_POST['fm_remember']) && $_POST['fm_remember'] == '1') {
                    $cookie_value = base64_encode($_POST['fm_usr'] . ':' . $auth_users[$_POST['fm_usr']]);
                    setcookie('fm_remember', $cookie_value, time() + (180 * 24 * 60 * 60), '/'); // 6 months
                }
                
                fm_set_msg(lng('You are logged in'));
                // Preserve the p parameter if it exists
                $redirect_path = isset($_GET['p']) ? '?p=' . urlencode($_GET['p']) : '';
                fm_redirect(FM_SELF_URL . $redirect_path);
            } else {
                unset($_SESSION[FM_SESSION_ID]['logged']);
                                ...
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    ...
                                    <div class="mb-3">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" id="fm_remember" name="fm_remember" value="1">
                                            <label class="custom-control-label" for="fm_remember"><?php echo lng('Remember me for 6 months') ?></label>
                                        </div>
                                    </div>                                        
